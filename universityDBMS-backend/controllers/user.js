const userModel = require('../models/user');
const tokenuser = require('../services/token');

const create = (req,res) =>{
    let userid = tokenuser.getUserId(req);
    let request = req.body;
    console.log('user controller', request);
    userModel.createUser(request.newdata)
    
    .then((result)=>{
        console.log('result successful',result)
        res.send(result)
    })
    .catch((err)=>{
        console.log('error is found',err)
        res.send(err)
    })

    
}

const get = (req, res) =>{  
    console.log('request query', req.query)
    if(Boolean(req.query.getUsers)){
        console.log('inside get all users')
        let userId = tokenuser.getUserId(req)
        userModel.getAllUsers(userId)
        .then((result) =>{
            res.send(result)
        })
        .catch((err)=>{
            console.log('error found in getAllUser', err)
            res.send(err)
        })
    }
    else if(req.query.getUserData){
        userModel.getUserData(req.query)
        .then((result) =>{
            res.send(result)
        })
        .catch((err)=>{
            console.log('error found in get', err)
            res.send(err)
        })
    }
    else{
        userModel.getUser(req.query)
    .then((result) =>{
        res.send(result)
    })
    .catch((err)=>{
        console.log('error found in get', err)
        res.send(err)
    })
    }
    
}


module.exports = {
    create, 
    get
};